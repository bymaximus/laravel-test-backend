		...
		<vs-row class="mb-2">
			...
			>
				<v-select
					...
					v-model="provedorListaRegistros.state.searchFields['id_imovel']['value']"
...
export default {
	...
	watch: {
		"provedorListaRegistros.state.searchFields.id_imovel.value"(val) {
	...
	methods: {
		...
		atualizarListaImoveis() {
			...
			this.provedorListaRegistros.busy = true;
			this.provedorListaRegistros.state.searchFields['id_imovel']['value'] = null;
			axios
				.get(this.backendUrl + "/listaContratado")
				.then(response => {
					if (
						response &&
						response.status == 200 &&
						...
						if (
							...
							this.listaImovel = response.data.imoveis;
						} else {
							this.$vs.notify({
								title: "Erro",
								text:
									"Incapaz de carregar a lista de imóveis.",
								iconPack: "feather",
								icon: "icon-alert-circle",
								color: "danger",
							});
