	...
	<div class="vx-card p-6">
		...
		<div class="bootstrap-inside">
			...
			>
				...
				<template v-slot:cell(dt_acesso)="row">
					<span class="text-nowrap">{{ row.value }}</span>
				</template>
				<template v-slot:cell(usuario)="row">
					<span class="text-break">{{ row.value }}</span>
				</template>
				<template v-slot:cell(id_perfil)="row">
					<span class="text-break">{{ row.value }}</span>
					<vx-tooltip
						color="danger"
						position="right"
						text="Perfil removido"
						v-show="(row.item && row.item.perfil && row.item.perfil.removido === true ? true : false)"
						:class="[
							{'disabled' : (provedorListaRegistros.busy || $parent.backendOcupado)},
						]"
					>
...
export default {
	...
	methods: {
		...
		processarRemoverRegistro() {
			...
			axios.delete(this.backendUrl + `/${this.registroSelecionado.id}`)
				.then((response) => {
					if (
						response &&
						response.status == 200 &&
