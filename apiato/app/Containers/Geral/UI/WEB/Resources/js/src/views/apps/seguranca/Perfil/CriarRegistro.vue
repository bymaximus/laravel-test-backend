			...
			<form @submit.prevent>
				...
				<div class="vx-row">
					<div class="vx-col w-full flex">
						<vs-checkbox
							v-model="camposCriar.historicoAlteracao"
							:disabled="backendOcupado"
	...
	computed: {
		...
		camposFormularioCriar() {
			return {
				...
				historico_alteracao: this.camposCriar.historicoAlteracao,
			};
		},
	},
	mounted() {
		if (!this.$acl.check("modificar")) {
			...
			axios
				.get(this.backendUrl + `/funcionalidades`)
				.then((response) => {
					if (
						response &&
						response.status == 200 &&
	...
	methods: {
		...
		processarCriar() {
			...
			this.$validator.validate().then((result) => {
				if (result) {
					var camposFormularioCriarClone = this.lodash.cloneDeep(
						this.camposFormularioCriar
					);
					camposFormularioCriarClone.funcionalidades = {};
					for (var key in this.funcionalidades) {
						if (
							this.funcionalidades[key] &&
							this.funcionalidades[key].hasOwnProperty(
							...
							if (this.funcionalidades[key].codigo > 0) {
								...
								camposFormularioCriarClone.funcionalidades = this.funcionalidadeFilhos(
									this.funcionalidades[key].children
								);
							}
						}
					...
					axios
						.post(this.backendUrl, camposFormularioCriarClone)
						.then((response) => {
							if (
								response &&
								response.status == 201 &&
