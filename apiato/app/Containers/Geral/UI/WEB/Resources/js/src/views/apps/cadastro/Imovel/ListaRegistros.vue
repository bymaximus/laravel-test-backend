<template>
	<div class="vx-card p-6">
		...
		<vs-row class="mb-2">
			...
			>
				<v-select
					name="id_estado_ativo"
					label="nome"
					placeholder="Filtrar por estado"
					v-model="provedorListaRegistros.state.searchFields['id_estado_ativo']['value']"
				...
				<template v-slot:cell(acoes)="row">
					<span class="text-nowrap">
						<vx-tooltip
							...
							position="left"
							text="Alterar"
							v-show="$acl.check('modificar') && row.item.hasOwnProperty('bloqueado') && row.item.bloqueado === false"
							:class="[
							{'disabled' : (provedorListaRegistros.busy || $parent.backendOcupado)},
						]"
						>
						...
						<vx-tooltip
							...
							position="left"
							text="Visualizar"
							v-show="$acl.check('modificar') && (!row.item.hasOwnProperty('bloqueado') || row.item.bloqueado === true)"
							:class="[
								{'disabled' : (provedorListaRegistros.busy || $parent.backendOcupado)},
							]"
						>
							<vs-button
								...
								class="dataTablesButton"
								v-show="$acl.check('modificar') && (!row.item.hasOwnProperty('bloqueado') || row.item.bloqueado === true)"
								@click="visualizarRegistro(row.item, row.index, $event.target)"
								:disabled="provedorListaRegistros.busy || $parent.backendOcupado"
							></vs-button>
						</vx-tooltip>
						<vx-tooltip
...
export default {
	...
	props: {
		...
		dialogoVisualizarRegistroAbrir: {
			type: Function,
			required: true,
		},
	...
	data() {
		...
		camposListaRegistros.splice(1, 1);
	...
	watch: {
		"provedorListaRegistros.state.searchFields.id_estado_ativo.value"(val) {
	...
	methods: {
		...
		visualizarRegistro(item, index, button) {
			if (
				this.provedorListaRegistros.busy ||
				this.$parent.backendOcupado ||
				this.removerRegistroDialogoStatus
			...
			this.dialogoVisualizarRegistroAbrir(item);
		},
