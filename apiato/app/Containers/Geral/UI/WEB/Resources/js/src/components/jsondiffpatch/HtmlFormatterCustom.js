...
class HtmlFormatterCustom extends formatters.html.default {
	...
	formatValue(context, value) {
		if (value &&
			...
			context.out(`<img src="${value.toString()}" />`);
		} else if (value &&
			value.toString().indexOf("\n") !== -1
		) {
			context.out(`<pre>${value.toString()}</pre>`);
		} else {
	...
	formatTextDiffString(context, value) {
		...
		for (let i = 0, l = lines.length; i < l; i++) {
			...
			for (
				let pieceIndex = 0, piecesLength = pieces.length; pieceIndex < piecesLength; pieceIndex++
			) {
				/* global decodeURI */
	...
	nodeBegin(context, key, leftKey, type, nodeType) {
      ...
      nodeType ? ` jsondiffpatch-child-node-type-${nodeType}` : ''
		...
		context.out(
			`<li class="${nodeClass} flex" data-key="${leftKey}">` +
			`<div class="jsondiffpatch-property-name flex-1">${leftKey}</div>`
		);
	}
	...
	format_unchanged(context, delta, left) {
		...
		context.out('<div class="jsondiffpatch-value flex-1">');
	...
	format_node(context, delta, left) {
		...
		context.out(
			`<li class="jsondiffpatch-modified flex">` +
			`<div class="flex-1"></div>` +
			`<div class="jsondiffpatch-value flex-1 jsondiffpatch-left-value">Valor Anterior (removido/alterado)</div>` +
			`<div class="jsondiffpatch-value flex-1 jsondiffpatch-right-value">Valor Novo (adicionado/alterado)</div>` +
			`</li>`
		);
	...
	format_modified(context, delta) {
		context.out('<div class="jsondiffpatch-value flex-1 jsondiffpatch-left-value">');
		this.formatValue(context, delta[0]);
		context.out(
			'</div>' + '<div class="jsondiffpatch-value flex-1 jsondiffpatch-right-value">'
		);
...
export default HtmlFormatterCustom;

let defaultInstance;

export function format(delta, left) {
	if (!defaultInstance) {
		defaultInstance = new HtmlFormatterCustom();
	}
