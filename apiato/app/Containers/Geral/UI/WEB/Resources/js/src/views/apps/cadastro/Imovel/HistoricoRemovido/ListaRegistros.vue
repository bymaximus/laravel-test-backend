	...
	<div class="vx-card p-6">
		...
		<div class="bootstrap-inside">
			...
			>
				...
				<template v-slot:cell(user.usuario)="row">
					<span class="text-break">{{ row.value }}</span>
					<vx-tooltip
						color="danger"
						position="right"
						text="Usuário removido"
						v-show="(row.item && row.item.user && row.item.user.removido === true ? true : false)"
						:class="[
							{'disabled' : (provedorListaRegistros.busy || $parent.backendOcupado)},
						]"
					>
...
export default {
	...
	methods: {
		cancelar() {
			...
			this.$router.push(
				this.$parent.configuracaoPagina.routerPath + "/removido"
			);
		},
...
.pagina#{$nomePagina}RemovidoHistorico {
	.bootstrap-inside {
...
.pagina#{$nomePagina}RemovidoHistorico .bootstrap-inside {
