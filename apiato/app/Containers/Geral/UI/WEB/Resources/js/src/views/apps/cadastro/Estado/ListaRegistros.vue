	...
	<div class="vx-card p-6">
		...
		>
			<VuePerfectScrollbar
				...
				:key="$vs.rtl"
			>
				Tem certeza que deseja remover esse {{ backendModel.toLowerCase() }}?
				<vs-alert
		...
		<vs-prompt
			...
			type="confirm"
			color="warning"
			@accept="processarSincronizar"
			@cancel="cancelarSincronizar"
			@close="fecharSincronizar"
			@keyup.esc.native="fecharSincronizar(true)"
			:is-valid="! $parent.backendOcupado && ! provedorListaRegistros.busy"
			:active.sync="sincronizarDialogoStatus"
			ref="sincronizarDialogo"
		>
			<VuePerfectScrollbar
				class="scroll-area p-4"
				:settings="scrollSettings"
				:key="$vs.rtl"
			>
				Tem certeza que deseja agendar a tarefa de sincronização de {{ backendModel.toLowerCase() }}?
				<vs-alert
					:active="(mensagemErro && mensagemErro != '' ? true : false)"
					color="danger"
					class="mb-10"
		...
		<vs-row class="mb-2">
			...
			>
				<vs-button
					...
					@click="sincronizarDialogo"
				>Sincronizar</vs-button>
			</vs-col>
			<vs-col
				vs-type="flex"
				vs-align="flex-end"
		...
		<div class="bootstrap-inside">
			...
			>
				...
				<template v-slot:cell(sigla)="row">
					<span class="text-nowrap">{{ row.value }}</span>
				</template>
					...
					<span class="text-nowrap">
						...
						>
							<vs-button
								...
								:disabled="provedorListaRegistros.busy || $parent.backendOcupado || removerRegistroDialogoStatus || sincronizarDialogoStatus"
							></vs-button>
						</vx-tooltip>
						<vx-tooltip
							color="danger"
...
export default {
	...
	data() {
		...
		return {
			...
			sincronizarDialogoStatus: false,
			scrollSettings: {
	...
	methods: {
		...
		cancelarSincronizar() {
			if (
				...
				this.sincronizarDialogoStatus = true;
				return;
			}
			this.mudarListaRegistrosOrdenacaoStatus(true);
		},
		...
		fecharSincronizar(evt) {
			if (
				...
				this.sincronizarDialogoStatus = false;
			}
			this.mudarListaRegistrosOrdenacaoStatus(true);
		},
			...
			this.$nextTick(() => {
				setTimeout(() => {
					window
						.jQuery(
							".vs-dialog-cancel-button",
							this.$refs.sincronizarDialogo.$refs.dialogx
						)
						.trigger("focus");
				}, 150);
			});
		...
		mudarSincronizarDialogoStatus(status) {
			if (status) {
				window
					.jQuery(
						".vs-dialog-cancel",
				...
				window
					.jQuery(
						".vs-dialog-dark",
						this.$refs.sincronizarDialogo.$refs.con
					)
					.removeClass("disabled");
				window
					.jQuery(
			...
			axios
				...
				.then((response) => {
					if (
						...
						this.$vs.notify({
							title: "Sucesso",
							text: `${this.backendModel} removido.`,
							iconPack: "feather",
							icon: "icon-check",
							color: "success",
						});
		...
		processarSincronizar() {
			...
			this.$parent.backendOcupado = true;
			this.mensagemErro = "";
			this.mudarSincronizarDialogoStatus(false);
			axios
				.post(this.backendUrl + `/sincronizar`)
				.then((response) => {
					if (
						response &&
						response.status == 200 &&
						...
						(
							...
							response.data.status == "existente"
						)
					) {
						if (response.data.status == "criado") {
							this.$vs.notify({
								title: "Sucesso",
								text: `Tarefa de sincronização de ${this.backendModel} agendada.`,
								iconPack: "feather",
								icon: "icon-check",
								color: "success",
							});
							...
							this.$vs.notify({
								title: "Sucesso",
								text: `Tarefa de sincronização de ${this.backendModel} já existe.`,
								iconPack: "feather",
								icon: "icon-check",
								color: "warning",
							});
						...
						this.$vs.notify({
							title: "Erro",
							text: "Dados de resposta da sincronização.",
							iconPack: "feather",
							icon: "icon-alert-circle",
							color: "danger",
						});
						this.$parent.backendOcupado = false;
						this.mudarSincronizarDialogoStatus(true);
					}
				})
				.catch((error) => {
					var errMsg = "Erro na requisição.";
