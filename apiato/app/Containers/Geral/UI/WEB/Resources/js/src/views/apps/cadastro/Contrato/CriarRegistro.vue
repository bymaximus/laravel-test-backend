				...
				<div class="vx-row mb-3">
					<div class="vx-col w-full">
						<label
							...
							:class="[
								{'input-select-label-success--active' : !errors.has('id_imovel') && camposCriar.idImovel && camposCriar.idImovel != '' && campoIdImovelFocado},
				...
				<div class="vx-row mb-3">
					<div class="vx-col w-1/2">
						<label
							...
							:class="[
								{'input-select-label-success--active' : !errors.has('tipo_pessoa') && camposCriar.tipoPessoa && camposCriar.tipoPessoa != ''},
								{'input-select-label-danger--active' : errors.has('tipo_pessoa')}
							]"
						>Tipo Pessoa</label>
						<vs-radio
							name="tipo_pessoa"
							v-model="camposCriar.tipoPessoa"
							:vs-value="1"
					...
					<div class="vx-col w-1/2">
						<vs-input
							name="documento"
							label="Documento"
							v-model="camposCriar.documento"
							class="w-full campoRequerido"
							v-validate.continues="{ required: true, validarTipoPessoaDocumento: {tipoPessoa: camposCriar.tipoPessoa, errors: this.errors} }"
							type="tel"
							...
							:success="!errors.has('documento') && camposCriar.documento != ''"
...
export default {
	...
	computed: {
		tipoPessoaMascara() {
			if (this.camposCriar.tipoPessoa == 1) {
				return '###.###.###-##';
			} else if (this.camposCriar.tipoPessoa == 2) {
				return '##.###.###/####-##';
			}
			return '';
		},
		...
		camposFormularioCriar() {
			return {
				id_imovel: this.camposCriar.idImovel,
				email: this.camposCriar.email,
				nome: this.camposCriar.nome,
				tipo_pessoa: this.camposCriar.tipoPessoa,
				documento: this.camposCriar.documento,
			};
		},
	},
	mounted() {
		...
		if (!this.$acl.check("modificar")) {
			...
			axios
				.get(this.backendUrl + "/listaDisponivel")
				.then(response => {
					if (
						response &&
						response.status == 200 &&
